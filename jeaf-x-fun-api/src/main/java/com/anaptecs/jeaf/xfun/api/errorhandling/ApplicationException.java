...
/**
 * Class is base class for so called application exceptions. Application exceptions are used to indicate problems
 * dealing with the application specific logic, e.g. some business constraints are not fulfilled. Application exceptions
 * must only be used in cases where application specific problems arise and never in the context of technical problems.
 * 
...
public abstract class ApplicationException extends Exception implements ExceptionInfoProvider {
   ...
   */
  protected ApplicationException( ErrorCode pErrorCode ) {
    this(pErrorCode, (Throwable) null, (String[]) null);
  }

  /**
   * Constructor initializes this application exception with the passed error code, message parameters.
   * 
   * @param pErrorCode Error code identifying the occurred problem. Among other things the error code is used to
   * determine the corresponding error message. The parameter must not be null.
   ...
   * null.
   */
  protected ApplicationException( ErrorCode pErrorCode, String... pMessageParameters ) {
   ...
   * null.
   */
  protected ApplicationException( ErrorCode pErrorCode, Throwable pCause, String... pMessageParameters ) {
   ...
   * null.
   */
  protected ApplicationException( ErrorCode pErrorCode, String pTechnicalDetails, Throwable pCause,
   ...
   * @param pCause Throwable object that caused this exception. The parameter may be null.
   * 
   * @deprecated Please use {@link ApplicationException#ApplicationException(ErrorCode, Throwable, String...)} instead.
   */
  @Deprecated
  protected ApplicationException( ErrorCode pErrorCode, String[] pMessageParameters, Throwable pCause ) {
    this(pErrorCode, pCause, pMessageParameters);
  }

  /**
   * Method returns an ErrorCode object that identifies an occurred exception. An exception can always be identified by
   * its error code.
   * 
  ...
  /**
   * Method returns an error message that describes the occurred exception. Therefore the provided message parameters
   * and the error code are used. The method always returns a message text that is localized using the current default
   * locale.
   * 
   * @return String Error massage describing the occurred exception. The method never returns null.
   */
  @Override
  ...
  /**
   * Method resolves the version of the software component to which the ApplicationException subclass belongs.
   * 
